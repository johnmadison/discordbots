...
client.on('ready', () => {
	...
	sendmsg("Roulette is Ready!", chans.bot);
});
...
function register(msg)
{
	...
	{
	 db.get('users')
	.push({ name: cmds[2], bal: 2000})
	.write();
	
	fs.writeFileSync('db.json', JSON.stringify(db.getState()));
	
	msg.reply("registered user "+ cmds[2]);
	return;
	}
...
function roulette(message,msg)
{
	...
	if (bet == "register" && adminUsers.includes(message.author.username) )
	{
			register(message);
			return;
	}
